...
const SEVERITY_STYLES = {
  critical: { color: '#c62828', bg: 'rgba(198,40,40,0.08)', label: '🔴 CRITICAL' },
  ...
  return (
    <div className={`notification-panel ${isOpen ? 'open' : ''}`}>
      <div className="notif-header">
        ...
        <button className="btn-text" onClick={() => dispatch({ type: 'CLEAR_NOTIFICATIONS' })}>
          Clear All
        </button>
      </div>
        ...
        <div className="notif-maint-section">
          <div className="notif-maint-header">
            ...
            <div className="notif-maint-badges">
              {criticalCount > 0 && (
                <span className="notif-maint-badge notif-maint-badge-critical">{criticalCount} Critical</span>
              )}
              {warningCount > 0 && (
                <span className="notif-maint-badge notif-maint-badge-warning">{warningCount} Warning</span>
              )}
              <span className="notif-maint-badge notif-maint-badge-total">{activeAlerts.length} Active</span>
            </div>
          </div>
          <div className="notif-maint-list">
            {activeAlerts.map(alert => {
              ...
              return (
                <div className="notif-maint-item" key={alert.id} style={{ borderLeftColor: sev.color }}>
                  <div className="notif-maint-item-top">
                    <span className="notif-maint-item-label">{alert.label}</span>
                    <span className="notif-maint-sev-badge" style={{ color: sev.color, background: sev.bg }}>
                      {alert.severity}
                    </span>
                  </div>
                  <p className="notif-maint-item-desc">{alert.reason || alert.desc}</p>
              ...
              return (
                ...
                >
                  <div className="notif-item-top">
                    <span className="notif-sev" style={{ color: sev.color, background: sev.bg }}>
                      {sev.label}
                    </span>
                  ...
                  {n.type === 'maintenance' ? (
                    <div className="notif-maint-details">
                      <p><strong>Reason:</strong> {n.msg.split(' Dispatch')[0] || n.msg.split(' Halt')[0] || n.msg.split(' Inspect')[0] || n.msg.split(' Check')[0] || n.msg.split(' Replenish')[0]}</p>
                      <p><strong>Recommended Action:</strong> {n.msg}</p>
                    </div>
                  ) : (
