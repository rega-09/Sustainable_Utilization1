...
const ALERT_RULES = [
  {
    ...
    check: (src) => (src.generation / src.capacity) < 0.60,
  },
  {
    id: 'wind_repair',
    source: 'wind',
    ...
    priority: 'critical',
    recommendedAction: 'Halt turbine and schedule immediate mechanical inspection.',
    check: (src) => (src.generation / src.capacity) < 0.30,
  },
  {
    id: 'hydro_maint',
    source: 'hydro',
    ...
    check: (src) => (src.generation / src.capacity) < 0.40,
  },
];
...
const initialState = {
  ...
  hubs: {
    hubA: {
      batteries: [
        { id: 'B1', capacity: 1000, stored: 780, health: 98, temperature: 24, cycles: 1102, status: 'charging', rate: 40 },
        { id: 'B2', capacity: 1000, stored: 640, health: 95, temperature: 25, cycles: 1250, status: 'charging', rate: 40 },
        { id: 'B3', capacity: 1000, stored: 910, health: 99, temperature: 22, cycles: 800, status: 'standby', rate: 0 },
      ],
    },
    hubB: {
      batteries: [
        { id: 'B1', capacity: 750, stored: 500, health: 96, temperature: 26, cycles: 1389, status: 'idle', rate: 30 },
        { id: 'B2', capacity: 750, stored: 400, health: 92, temperature: 27, cycles: 1500, status: 'idle', rate: 30 },
        { id: 'B3', capacity: 750, stored: 700, health: 99, temperature: 23, cycles: 400, status: 'idle', rate: 30 },
      ],
    },
  },
  ...
  logs: [],
  notifications: [
    { id: 1, title: 'System Online', msg: 'Smart Renewable Energy Management Platform initialized.', time: 'Just now', read: false, severity: 'info' },
  ],
  maintenanceAlerts: [],
...
function syncAggregateBattery(hubs) {
  let totalCap = 0, totalStored = 0, totalHealth = 0, totalTemp = 0, totalCycles = 0, totalRate = 0, count = 0;
  let anyCharging = false, anyDischarging = false;
...
function energyReducer(state, action) {
  switch (action.type) {
    ...
    case 'TOGGLE_SOLAR_SIMULATION': {
      return {
        ...state,
        solarSimulation: { ...state.solarSimulation, isPlaying: !state.solarSimulation.isPlaying },
      };
    }
    ...
    case 'RESET_SOLAR_SIMULATION': {
      ...
      return {
        ...state,
        sources: newSources,
        solarSimulation: { ...sim, currentSlot: slot, intensity, isPlaying: false },
      };
    }
      ...
      Object.entries(HUBS).forEach(([hubId, hubDef]) => {
        ...
        const newBatteries = newHubs[hubId].batteries.map(bat => {
          ...
          if (hubTarget > 0) {
            ...
            return { ...bat, stored: bat.stored + toStore, status: toStore > 0 ? 'charging' : 'standby' };
          } else {
            const toDischarge = Math.min(remainingTarget, bat.stored, bat.rate);
            remainingTarget -= toDischarge;
            return { ...bat, stored: bat.stored - toDischarge, status: toDischarge > 0 ? 'discharging' : 'standby' };
          }
        });
    ...
    case 'HUB_CHARGE': {
      ...
      const hubGen = hubDef.sources.reduce((s, sk) => s + state.sources[sk].generation, 0);
      ...
      const newBatteries = hub.batteries.map(bat => {
        ...
        return { ...bat, status: bat.stored >= bat.capacity ? 'standby' : 'idle' };
      });

      const newHubs = { ...state.hubs, [hubId]: { ...hub, batteries: newBatteries } };
    ...
    case 'HUB_DISCHARGE': {
      ...
      const newBatteries = hub.batteries.map(bat => {
        ...
        if (dischargeAmount > 0) {
          return { ...bat, stored: bat.stored - dischargeAmount, status: 'discharging' };
        }
    ...
    case 'HUB_STOP': {
      const hubId = action.payload;
      const hub = state.hubs[hubId];
      const newBatteries = hub.batteries.map(bat => ({ ...bat, status: 'standby' }));
      const newHubs = { ...state.hubs, [hubId]: { ...hub, batteries: newBatteries } };
      return { ...state, hubs: newHubs };
    }
      ...
      ALERT_RULES.forEach(rule => {
        ...
        if (rule.check(src)) {
          ...
          if (!newCooldowns[rule.id] || (now - newCooldowns[rule.id]) > 60000) {
            const existingIdx = newAlerts.findIndex(a => a.ruleId === rule.id && !a.resolved);
            if (existingIdx === -1) {
              const alert = {
                id: now + Math.random(),
                ruleId: rule.id,
    ...
    case 'RECORD_HISTORY': {
      ...
      return {
        ...
        history: {
          ...
          consumption: [...state.history.consumption, state.consumption].slice(-60),
          battery: [...state.history.battery, batPercent].slice(-60),
          timestamps: [...state.history.timestamps, new Date().toLocaleTimeString()].slice(-60),
        },
      };
    }
...
export function getCO2Saved(source, generation) {
  ...
  return (generation * 0.72).toFixed(0);
}
...
export function getSustainabilityScore(state) {
  ...
  return {
    total: Math.round((renewPct * 0.3 + efficiency * 0.25 + Math.min(storage, 100) * 0.2 + stability * 0.25)),
